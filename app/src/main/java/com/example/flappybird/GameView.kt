...
class GameView(context: Context, val viewModel: MainMenuViewModel) : SurfaceView(context), Runnable, SurfaceHolder.Callback {
    ...
    val textPaint = Paint().apply {
        ...
        isAntiAlias = true // importante para fuentes pixel
    }
    ...
    private fun drawGame()
    {
        ...
        val text_1 = "Points:"
        val textWidth_1 = textPaint.measureText(text_1)
    ...
    fun saveHighScore() {
        ...
        val currentHigh = viewModel.uiState.value.score
        val userId = prefs.getString("userId", null)

        Log.d("gameview", currentHigh + " : " + points)


        if (points.toInt() > currentHigh.toInt()) {
            viewModel.saveScore(points)
        }
    }
}
